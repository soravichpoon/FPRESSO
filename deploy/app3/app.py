...
def login():
    ...
    auth_response = requests.post('https://auth-service2-swlzbjlflq-as.a.run.app/authenticate', json={'username': username, 'password': password, 'role': user['role'], 'appNo' : 'app3'})
...
def protected():
    ...
    if token:
        verify_response = requests.get('https://sso-service-swlzbjlflq-as.a.run.app/verify', headers={'appNo': 'app3'}, cookies={'sso_token': token})
