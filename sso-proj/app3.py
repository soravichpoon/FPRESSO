...
def get_cookie1():
    ...
    resp = make_response(redirect('https://app1-swlzbjlflq-as.a.run.app/gen_cookie2'))
...
def gen_cookie():
    token = request.cookies.get('sso_token')
    resp = make_response(redirect('https://app1-swlzbjlflq-as.a.run.app/protected'))
    resp.set_cookie('sso_token', token, httponly=True, secure=True, samesite='Lax')
    return resp
...
def gen_cookie2():
    token = request.cookies.get('sso_token')
    resp = make_response(redirect('https://app2-swlzbjlflq-as.a.run.app/protected'))
    resp.set_cookie('sso_token', token, httponly=True, secure=True, samesite='Lax')
    return resp
